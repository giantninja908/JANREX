...
module.exports = () => {
  if (!config.scriptSettings.autoFFA.enabled)
    return;

  window.addEventListener('keyup', e => {
    if (e.key.toUpperCase() ===
        config.scriptSettings.autoFFA.autoJoinKeybind.toUpperCase()) {
  ...
  window.fetch = async (...args) => {
    ...
    var gameList = resGL.games.filter(
        game =>
            ...
            game[4].g === gameModes[config.scriptSettings.autoFFA
                                        .autoJoinGameMode] && // Gamemode check
            (config.scriptSettings.autoFFA.attemptJoinFull ||
             game[2] <
                 game[3]) // Full game check, attemptJoinFull bypasses this
    )

    var regionFilteredGames =
        gameList.filter(game => game[1] === region); // Filter by region
    if (!(config.scriptSettings.autoFFA.filterWrongVersion) ||
        regionFilteredGames.length !== 0) {
